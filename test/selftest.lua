-- compiles and runs a test file designed to test all of clua's main features
...
local _,err=pcall(function()shell.run('clua test1 output --from-path:/clua/test/ --to-path:/clua/test/ --log --no-debug --silent')shell.run('/clua/test/output')end)
if err then term.clear()term.setCursorPos(1,2)print(' CLua '..(CLUA_VERSION or'MISSING_VERSION_INFO')..' Copyright 2014 Skwerlman\n An Error Has Occured!\n\n')printError(' '..tostring(err):gsub('\n','\n ')..'\n\n')print(' '..(CLUA_LOG or'clua.log')..' may contain more info.\n Please wait...\n')sleep(3)end
